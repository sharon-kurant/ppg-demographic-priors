...
from ppg_bp_incremental.models.encoders.papagei import (
    ...
    PapageiEncoder,
    papagei_preprocessing_shape_trace,
    preprocess_papagei,
...
def test_papagei_ppgbp_drops_final_raw_sample_before_all_other_operations():
    time = np.arange(2100) / 1000
    signal = (
    ...
    released = preprocess_papagei(signal, 1000, dataset="PPG-BP")
    ...
    untrimmed_adaptation = preprocess_papagei(
        signal,
        1000,
...
def test_papagei_final_sample_trim_is_ppgbp_only_and_visible_in_trace():
    signal = np.linspace(-1.0, 1.0, 2100, dtype=np.float32)
    ppgbp_trace = papagei_preprocessing_shape_trace(
        signal,
        1000,
        dataset="PPG-BP",
    )
    pulsedb_trace = papagei_preprocessing_shape_trace(
        signal,
        1000,
        dataset="PulseDB-Vital",
    )
...
def test_papagei_ppgbp_trim_has_a_dataset_specific_cache_fingerprint(variant):
    encoder = PapageiEncoder.__new__(PapageiEncoder)
    encoder.variant = variant
    encoder.repository_commit = "0c537dad4d2850e15b724260de820dd68d77f0b0"
    encoder.checkpoint_sha256 = "3" * 64
